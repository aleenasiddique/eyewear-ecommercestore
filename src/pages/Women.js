...
export default function Women(){
  ...
  const displayedWomen = collection.filter(product => product.type === 'Women')
    const product = displayedWomen.map(product =>(
    ...
    return (
        <div>
              ...
              className="h-40 w-full bg-gradient-to-r from-green via-pink to-green px-8 md:px-16 flex flex-col  justify-center text-dark-green border-t border-white/30">
                <motion.p className="text-4xl font-extrabold italic tracking-wide">SHOP</motion.p>
                <motion.p className="font-semibold ">Home / Women's Collection</motion.p>
            </motion.div>
